...
@WebMvcTest(controllers=IspController.class)
public class TestIspApi {
	...
	public void testGetAllContacts() throws Exception {
		...
		List<IspModel> c=new ArrayList<>();
		c.add(new IspModel());
		
		when(this.ispSer.getAll()).thenReturn(c);
		this.mockMvc.perform(get("/abc")).andExpect(status().isOk()).andDo(print());
	}
	@Test
	public void testGetPlanByTitle() throws Exception{
		assertThat(this.ispSer).isNotNull();
		IspModel e= new IspModel();
		
		when(this.ispSer.getByTitle("Plan1")).thenReturn(e);
